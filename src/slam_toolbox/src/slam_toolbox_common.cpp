...
/*****************************************************************************/
{
  ...
  static double min_dist2 =
    ...
    smapper_->getMapper()->getParamMinimumTravelDistance();
  static double min_heading =
    smapper_->getMapper()->getParamMinimumTravelHeading();
  ...
  // Check whether either translation or heading changed enough.  The
  // upstream 2.6.10 pre-gate only checked translation, even though Karto's
  // own gate accepts heading OR distance.  That discarded every useful scan
  // during an in-place turn before Karto could evaluate it.
  const double dist2 = last_pose.SquaredDistance(pose);
  const double heading_delta = std::atan2(
    std::sin(pose.GetHeading() - last_pose.GetHeading()),
    std::cos(pose.GetHeading() - last_pose.GetHeading()));
  const bool translation_small = dist2 < 0.8 * min_dist2;
  const bool heading_small = std::fabs(heading_delta) < 0.8 * min_heading;
  if ((translation_small && heading_small) || scan_ctr < 5) {
    return false;
  }
