...
namespace lslidar_driver {
    ...
    void LslidarX10Driver::publishLiadrData(double completed_stamp) {
        ...
        if (publish_scan) {
            ...
            scan_msg->header.stamp = rclcpp::Time(
                static_cast<int64_t>(completed_stamp * 1000000000.0));
    ...
    bool LslidarX10Driver::poll() {
        ...
        if (end_fir_idx != actual_points) {
            ...
            // Store the timestamp with the completed message before handing it
            // to the publisher thread. point_cloud_time is seconds whereas
            // rclcpp::Time(int64_t) expects nanoseconds.
            if (publish_multiecholaserscan && lidar_model == "N10Plus") {
                multiecho_scan_->header.stamp = rclcpp::Time(
                    static_cast<int64_t>(point_cloud_time * 1000000000.0));
            }
            ...
            const double completed_stamp = point_cloud_time;
            thread_pool_->enqueue(
                [this, completed_stamp]() {
                    publishLiadrData(completed_stamp);
                });
            if(publish_multiecholaserscan && lidar_model == "N10Plus") {
